...
function showToast(message, type = "info") {
  ...
  $("#generate-report").click(async function () {
    showToast("Iniciando generación de informe...", "info");
    ...
    if (!startDate || !endDate) {
      ...
      showToast("Fechas no seleccionadas", "warning");
    ...
    if (selectedGroupIds.length === 0) {
      ...
      showToast("No se seleccionó ningún grupo", "warning");
      $("#generate-report").prop("disabled", false);
      $("#loading-icon").hide();
      $("#progress-container").hide();
      return;
    ...
    if (allHostIds.length === 0) {
      showToast("No hay hosts en los grupos seleccionados", "error");
      $("#generate-report").prop("disabled", false);
      $("#loading-icon").hide();
      $("#progress-container").hide();
      return;
    ...
    showToast(`Obteniendo items ping para ${allHostIds.length} hosts...`, "info");
    const pingItemsResp = await fetchPingItemsForHosts(allHostIds);
    const pingItems = pingItemsResp.result || [];

    if (pingItems.length === 0) {
      showToast("No hay items de ping para los hosts seleccionados", "error");
      $("#generate-report").prop("disabled", false);
      $("#loading-icon").hide();
      $("#progress-container").hide();
      return;
    ...
    showToast(`Obteniendo tendencias para ${itemIds.length} items...`, "info");
