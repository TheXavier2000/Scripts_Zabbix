    ...
    foreach ($groupId in $validGroupIds) {
        try {
            $hostsCountRes = Invoke-Zabbix @(
                @{ jsonrpc = "2.0"; method = "host.get"; params = @{ output = @("hostid"); groupids = $groupId };  id = 900 + $groupId }
            )
    ...
    foreach ($groupId in $validGroupIds) {
        ...
        try {
            $hostsRes = Invoke-Zabbix @(
                @{ jsonrpc = "2.0"; method = "host.get"; params = @{ output = @("hostid", "host", "name"); selectInterfaces = @("ip"); selectGroups = @("name"); groupids = $groupId };  id = 100 + $groupId }
            )
        } catch {
        ...
        try {
            $itemRes = Invoke-Zabbix @(
                @{ jsonrpc = "2.0"; method = "item.get"; params = @{ output = @("itemid", "hostid", "key_"); hostids = $allHostIds; filter = @{ key_ = "icmpping" } };  id = 101 + $groupId }
            )
        } catch {
        ...
        try {
            $historyRes = Invoke-Zabbix @(
                @{ jsonrpc = "2.0"; method = "history.get"; params = @{ output = "extend"; history = 3; itemids = $itemIds; time_from = $timeFrom; time_till = $timeTill; sortfield = "clock"; sortorder = "ASC"; limit = 100000 };  id = 102 + $groupId }
            )
        } catch {
        ...
        try {
            $problemRes = Invoke-Zabbix @(
                @{ jsonrpc = "2.0"; method = "problem.get"; params = @{ output = @("eventid", "name", "clock", "r_eventid", "hostid"); hostids = $allHostIds; time_from = $timeFrom; time_till = $timeTill; selectRelatedObject = "extend" };  id = 103 + $groupId }
            )
        } catch {
        ...
        foreach ($zbxhost in $hosts) {
            ...
            if ($disponibilidadPromedio -isnot [string]) {
                ...
                # Siempre agregamos el host para mostrarlo, usando 0 si no hay datos
                $hostsDisponibilidades += [PSCustomObject]@{
                    ...
                    Disponibilidad = if ($disponibilidadPromedio -is [string]) { 0 } else { $disponibilidadPromedio }
                }
...
$top10 = $hostsDisponibilidades |
    ...
    ForEach-Object {
        [PSCustomObject]@{
            ...
            IP             = $_.IP
            Grupo          = $_.Grupo
            Disponibilidad = if ($_.Disponibilidad -eq 0) { "Sin datos" } else { "$($_.Disponibilidad)%" }
        }
    }
...
if ($promediosPorGrupoConRango -and $promediosPorGrupoConRango.Count -gt 0) {
    ...
    Write-Warning "No hay datos para promedio por grupo con rango, no se genera la tabla."
}
